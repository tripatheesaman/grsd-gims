...
import { AssetsReportResponse, AssetsReportRow } from '@/types/assetsReport';
...
export default function AssetsReportPage() {
    ...
    const [data, setData] = useState<AssetsReportRow[]>([]);
    ...
    const fetchReport = useCallback(async () => {
        ...
        try {
            ...
            const response = await API.get<AssetsReportResponse>('/api/report/assets', { params });
    ...
    const sortIndicator = (field: string) =>
        sortBy === field ? (
            <span className="ml-1 text-[#003594]">{sortOrder === 'ASC' ? '↑' : '↓'}</span>
        ) : null;
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            const response = await API.post('/api/report/assets/export', payload, {
                responseType: 'blob',
            });
            ...
            link.href = url;
            link.download = `Assets_Report_${selectedFy.replace('/', '-')}.xlsx`;
            ...
            showSuccessToast({
                title: 'Exported',
                message: 'Assets report downloaded successfully',
                duration: 3000,
            });
            ...
            showErrorToast({
                title: 'Error',
                message: 'Failed to export assets report',
                duration: 5000,
            });
        } finally {
            setExporting(false);
    ...
    return (
        <div className="container mx-auto px-4 py-8 space-y-6">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="text-gray-600 mt-1">
                        Equipment valuation as of the selected fiscal year. Assets purchased after the selected FY are excluded.
                    </p>
                </div>
                <Button
                    onClick={() => setIsExportModalOpen(true)}
                    className="bg-[#003594] hover:bg-[#002a6e]"
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="text-lg text-[#003594]">Filters</CardTitle>
                </CardHeader>
                <CardContent>
                    <form onSubmit={handleSearch} className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div className="flex items-end gap-2 md:col-span-2 lg:col-span-4">
                            <Button type="submit" className="bg-[#003594] hover:bg-[#002a6e]">
            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                    <CardTitle className="text-lg text-[#003594]">
                        Assets as of FY {selectedFy || '—'}
                    </CardTitle>
                ...
                <CardContent>
                    {isLoading || fyLoading ? (
                        ...
                        <p className="text-center text-gray-500 py-12">No assets found for the selected fiscal year.</p>
                    ) : (
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        ...
                                        <TableHead
                                            className={sortableHeadClass}
                                            onClick={() => handleSort('purchase_fy')}
                                        >
                                            Purchase FY{sortIndicator('purchase_fy')}
                                        </TableHead>
                                        <TableHead
                                            className={`text-right ${sortableHeadClass}`}
                                            onClick={() => handleSort('original_purchase_cost_npr')}
                                        >
                                            Purchase Cost{sortIndicator('original_purchase_cost_npr')}
                                        </TableHead>
                                        <TableHead
                                            className={`text-right ${sortableHeadClass}`}
                                            onClick={() => handleSort('total_depreciation_npr')}
                                        >
                                            Total Depreciation{sortIndicator('total_depreciation_npr')}
                                        </TableHead>
                                        <TableHead
                                            className={`text-right ${sortableHeadClass}`}
                                            onClick={() => handleSort('current_value_npr')}
                                        >
                                            Current Value{sortIndicator('current_value_npr')}
                                        </TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                        ...
                                        return (
                                            <Fragment key={row.id}>
                                                <TableRow className="hover:bg-slate-50">
                                                    ...
                                                    <TableCell>{row.purchase_fy}</TableCell>
                                                    <TableCell className="text-right">
                                                        {formatNpr(row.original_purchase_cost_npr)}
                                                    </TableCell>
                                                    <TableCell className="text-right">
                                                        {formatNpr(row.total_depreciation_npr)}
                                                    </TableCell>
                                                    <TableCell className="text-right font-semibold text-[#003594]">
                                                        {formatNpr(row.current_value_npr)}
                                                    </TableCell>
                                                </TableRow>
                                                {isExpanded && (
                                                    <TableRow className="bg-slate-50/80">
                                                        <TableCell colSpan={8} className="p-4">
                                                            <div className="grid gap-4 md:grid-cols-2">
                                                                <div>
                                                                    <h4 className="text-sm font-semibold text-[#003594] mb-2">
                                                                        Equipment details
                                                                    </h4>
                                                                    <dl className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                                                                        <dt className="text-gray-500">Location</dt>
                                                                        <dd>{row.location || '—'}</dd>
                                                                        <dt className="text-gray-500">RRP Status</dt>
                                                                        <dd>{row.rrp_status || '—'}</dd>
                                                                        <dt className="text-gray-500">Servicability</dt>
                                                                        <dd>{row.servicability_status || '—'}</dd>
                                                                ...
                                                                <div>
                                                                    <h4 className="text-sm font-semibold text-[#003594] mb-2">
                                                                        Depreciation by FY (up to {selectedFy})
                                                                    </h4>
                                                                    <div className="overflow-x-auto">
                                                                        <Table>
                                                                            <TableHeader>
                                                                                    ...
                                                                                    <TableRow key={d.fy}>
                                                                                        ...
                                                                                        <TableCell className="text-right">
                                                                                            {d.annualDepreciationNpr > 0
                                                                                                ? formatNpr(d.annualDepreciationNpr)
                                                                                                : '—'}
                                                                                        </TableCell>
                                                                                        <TableCell className="text-right">
                                                                                            {formatNpr(d.bookValueEndOfFy)}
                                                                                        </TableCell>
                                                                                    </TableRow>
                                                                                ))}
                                                                            </TableBody>
            ...
            {fiscalYearColumns.length > 0 && (
                <p className="text-xs text-gray-500 text-center">
                    Excel export includes annual depreciation columns for FY{' '}
            ...
            <Dialog open={isExportModalOpen} onOpenChange={setIsExportModalOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Export Assets Report</DialogTitle>
                        <DialogDescription>
                            Download equipment details, purchase costs, per-FY depreciation, and current values for FY {selectedFy}.
                        </DialogDescription>
                    </DialogHeader>
                    <RadioGroup value={exportType} onValueChange={setExportType} className="space-y-3">
                        <div className="flex items-center space-x-2">
                            <RadioGroupItem value="all" id="export-all" />
                            <Label htmlFor="export-all">All matching assets ({total})</Label>
                        </div>
                        <div className="flex items-center space-x-2">
