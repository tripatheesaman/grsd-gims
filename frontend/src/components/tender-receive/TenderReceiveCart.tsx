...
interface TenderReceiveCartProps {
...
function CartItem({ item, onEdit, onDelete }: {
    ...
    onEdit: (item: TenderReceiveCartItem) => void;
...
export function TenderReceiveCart({ items, onUpdateItem, onDeleteItem, onSubmit, isSubmitDisabled, isSubmitting, }: TenderReceiveCartProps) {
    const [editingItem, setEditingItem] = useState<TenderReceiveCartItem | null>(null);
    const [isEditDialogOpen, setIsEditDialogOpen] = useState(false);
    const [editFormData, setEditFormData] = useState<Partial<TenderReceiveCartItem>>({});
    const handleEdit = (item: TenderReceiveCartItem) => {
        setEditingItem(item);
    ...
    return (<div className="space-y-4">
      ...
      {items.length === 0 ? (<p className="text-muted-foreground">No items in list</p>) : (<div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input id="editReceiveQuantity" type="number" value={editFormData.receiveQuantity} onChange={(e) => setEditFormData(prev => ({ ...prev, receiveQuantity: Number(e.target.value) }))} className="mt-1 border-[#002a6e]/10 focus:border-[#003594] focus:ring-[#003594]/20" min="1" required/>
                </div>
                <div>
