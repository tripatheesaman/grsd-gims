            ...
            <DialogContent className="max-w-4xl bg-white rounded-lg shadow-xl border-[#002a6e]/10">
                <DialogHeader className="pb-4 border-b border-[#002a6e]/10">
                    <DialogTitle className="text-2xl font-bold bg-gradient-to-r from-[#003594] to-[#d2293b] bg-clip-text text-transparent">
                        {isCapital ? 'RRCP' : 'RRP'} Preview — {rrp.rrpNumber}
                    </DialogTitle>
                </DialogHeader>
                <div className="space-y-8 py-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <span
                                className={cn(
                                    ...
                                    isCapital && 'bg-purple-100 text-purple-800',
                                    !isCapital && rrp.currency === 'NPR' && 'bg-green-100 text-green-800',
                                    !isCapital && rrp.currency !== 'NPR' && 'bg-blue-100 text-blue-800'
                                )}
                            >
                                {isCapital ? 'CAPITAL (RRCP)' : rrp.currency === 'NPR' ? 'LOCAL' : 'FOREIGN'}
                            </span>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p className="text-sm">
                                {rrp.referenceDoc ? 'Uploaded' : rrp.approvalStatus === 'APPROVED' ? 'Not uploaded' : 'Available after approval'}
                            </p>
                        </div>
                    </div>
                            ...
                            <table className="w-full min-w-[700px]">
                                <thead className="bg-[#003594]/5">
                                    <tr>
                                        {isCapital ? (
                                            <>
                                                <th className="px-6 py-3 text-left text-xs font-semibold text-[#003594] uppercase">GE No.</th>
                                                <th className="px-6 py-3 text-left text-xs font-semibold text-[#003594] uppercase">Equipment</th>
                                                <th className="px-6 py-3 text-left text-xs font-semibold text-[#003594] uppercase">Model / Serial</th>
                                            </>
                                        ) : (
                                            <>
                                        ...
                                        <th className="px-6 py-3 text-left text-xs font-semibold text-[#003594] uppercase">Total (NPR)</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {rrp.items.map((item) => (
