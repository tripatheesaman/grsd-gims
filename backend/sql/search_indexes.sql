-- Optional manual migration: search performance indexes
-- These are also applied automatically on backend startup via ensureAssetSpareSchema().

-- stock_details: denormalized search blob (prefix-friendly)
...
-- MySQL does not support CREATE INDEX IF NOT EXISTS; indexes below are created on
-- backend startup via ensureAssetSpareSchema(). Run manually only if needed.
