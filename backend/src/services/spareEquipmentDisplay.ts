...
export const SPARE_STOCK_JOIN = `
  LEFT JOIN spare_compatibility sc
    ON sc.nac_code COLLATE ${COLLATE} = sd.nac_code COLLATE ${COLLATE}
  LEFT JOIN assets a
...
const equipmentNameExistsSql = (stockAlias: string, param: string) => `
  EXISTS (
    ...
    INNER JOIN assets a_n
      ON a_n.equipment_code COLLATE ${COLLATE} = sc_n.equipment_code COLLATE ${COLLATE}
    WHERE sc_n.nac_code COLLATE ${COLLATE} = ${stockAlias}.nac_code COLLATE ${COLLATE}
      AND a_n.name COLLATE ${COLLATE} LIKE ${param}
  )`;
...
const applicableAssetNameExistsSql = (stockAlias: string, param: string) => `
  EXISTS (
    ...
    WHERE a_eq.name COLLATE ${COLLATE} LIKE ${param}
      AND (
        FIND_IN_SET(
          a_eq.equipment_code COLLATE ${COLLATE},
          REPLACE(${stockAlias}.applicable_equipments, ' ', '') COLLATE ${COLLATE}
        ) > 0
        OR ${stockAlias}.applicable_equipments COLLATE ${COLLATE} LIKE CONCAT('%,', a_eq.equipment_code, ',%')
        OR ${stockAlias}.applicable_equipments COLLATE ${COLLATE} LIKE CONCAT(a_eq.equipment_code, ',%')
        OR ${stockAlias}.applicable_equipments COLLATE ${COLLATE} LIKE CONCAT('%,', a_eq.equipment_code)
        OR ${stockAlias}.applicable_equipments COLLATE ${COLLATE} = a_eq.equipment_code COLLATE ${COLLATE}
      )
  )`;
...
export const appendEquipmentFilter = (
    ...
    if (useJoin) {
      ...
      OR a.name COLLATE ${COLLATE} LIKE ?
...
export const equipmentDisplaySubquery = (
  ...
  LEFT JOIN assets a2
    ON a2.equipment_code COLLATE ${COLLATE} = sc2.equipment_code COLLATE ${COLLATE}
  WHERE sc2.nac_code COLLATE ${COLLATE} = ${nacCodeColumn} COLLATE ${COLLATE}
)`;
