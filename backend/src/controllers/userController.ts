...
export const getUsers = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const [currentUserRole] = await pool.query<Role[]>(`SELECT r.heirarchy 
             FROM users u 
        ...
        const [users] = await pool.query<UserWithRole[]>(`SELECT u.*, r.role_name, r.heirarchy
             FROM users u
             JOIN roles r ON u.role_id = r.role_id
             WHERE r.heirarchy >= ?
...
export const createUser = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const [existingUser] = await connection.query<User[]>('SELECT id FROM users WHERE username = ?', [username]);
        ...
        const [result] = await connection.query(`INSERT INTO users (
                ...
                created_by,can_reset_password, created_at, updated_at
        ...
        const [rolePermissions] = await connection.query<Role[]>('SELECT permission_id FROM roles WHERE role_id = ?', [role]);
...
export const updateUser = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const [existingUser] = await connection.query<User[]>('SELECT id FROM users WHERE username = ? AND id != ?', [username, id]);
        if (existingUser.length > 0) {
            await connection.rollback();
        ...
        const [currentUserRole] = await connection.query<Role[]>('SELECT role_id FROM users WHERE id = ?', [id]);
        if (currentUserRole.length === 0) {
            await connection.rollback();
...
export const deleteUser = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const { id } = req.params;
        const [existingUser] = await connection.query<User[]>('SELECT id, username FROM users WHERE id = ?', [id]);
        if (existingUser.length === 0) {
            await connection.rollback();
...
export const updateUserPermissions = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const [existingUser] = await connection.query<User[]>('SELECT id, username FROM users WHERE id = ?', [userId]);
        if (existingUser.length === 0) {
            await connection.rollback();
