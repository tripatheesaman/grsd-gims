...
export const login = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const accessToken = jwt.sign({ UserInfo: userInfo }, process.env.ACCESS_TOKEN_SECRET as string, { expiresIn: "7h" });
...
export const refreshToken = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const newAccessToken = jwt.sign({ UserInfo: userInfo }, process.env.ACCESS_TOKEN_SECRET as string, { expiresIn: "7h" });
...
export const checkResetEligibility = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        const [users] = await pool.execute<RowDataPacket[]>('SELECT can_reset_password FROM users WHERE username = ?', [email]);
        if (users.length === 0) {
            res.status(404).json({
                error: 'Not Found',
                message: 'User not found'
...
export const resetPassword = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        const [users] = await pool.execute<RowDataPacket[]>('SELECT id, can_reset_password FROM users WHERE username = ?', [email]);
        if (users.length === 0) {
            res.status(404).json({
                error: 'Not Found',
                message: 'User not found'
