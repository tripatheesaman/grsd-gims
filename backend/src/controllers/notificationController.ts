...
export const getUserNotifications = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const [users] = await connection.query<User[]>('SELECT id FROM users WHERE username = ?', [username]);
        if (users.length === 0) {
...
export const markNotificationAsRead = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const [result] = await connection.query<ResultSetHeader>('UPDATE notifications SET is_read = TRUE WHERE id = ?', [notificationId]);
...
export const deleteNotification = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        const { notificationId } = req.params;
        const [result] = await connection.query<ResultSetHeader>('DELETE FROM notifications WHERE id = ?', [notificationId]);
        if (result.affectedRows === 0) {
